 ...
 * @param {Array} [optional] ext more extensions
 * @param {Regex} [optional] excludes regex used for exclude folders
 * @api private
 */
function watch(io, root, ext ,excludes){
...
module.exports = function(opt) {
  ...
  var code = uglify.minify(path.resolve(__dirname, 'reload.js')).code;
  var temp = '<script type="text/javascript">' + code + '</script>';
  if(socket === true){
  ...
  //watch file change
  watch(opt.io, opt.root, opt.ext, opt.excludes);
  /**
